...
I tested whether egg number and egg size changed across successive clutches by fitting a multivariate Bayesian mixed-effects model that included clutch order (first vs. second clutch) as a fixed effect for both traits. Mean egg size was analysed assuming a Gaussian error distribution with an identity link, whereas egg number was analysed using a Poisson error distribution with a log link.
The two traits were modelled jointly to account for potential covariance at the female level. For each trait, the model included a fixed intercept and an effect of clutch order. Female identity was included as a random intercept for both egg size and egg number to account for repeated measures across clutches. The female-specific random intercepts for the two traits were allowed to covary, thereby estimating the among-female variance–covariance structure between egg size and egg number.
Residual correlations between traits were not estimated (set_rescor(FALSE)), such that covariance between traits was modelled only at the female level. This model therefore tested whether egg size and egg number differed systematically between first and second clutches while accounting for repeated measures and among-female variation in both traits.

I calculated the raw phenotypic association between egg number and egg size across all observations using Pearson’s correlation coefficient (*r*). This estimate reflects the overall linear association between traits without accounting for repeated measures or hierarchical structure. To quantify among-female covariation, I calculated the Pearson correlation between female mean egg number and female mean egg size. Female means were calculated across broods. This approach isolates between-female covariance independent of within-female variation. To partition covariance into among-female and residual components, I fitted a multivariate mixed-effects model treating egg size and egg number as Gaussian responses. Female identity was included as a random intercept for both traits, and residual correlations were estimated (`set_rescor(TRUE)`). This allowed covariance between traits to be decomposed into an among-female component, reflecting consistent differences among individuals, and a residual component, reflecting clutch-level associations within females. Female-specific random intercepts for egg size and egg number were allowed to correlate, providing an estimate of the among-female correlation. Residual errors were also allowed to correlate, yielding an estimate of the within-female (residual) association between traits. To further distinguish plastic from strategic effects, I followed van de Pol and Wright (2009) by decomposing egg number into a within-female component (deviation from a female’s own mean) and a between-female component (a female’s mean relative to the population mean). I then fitted a linear mixed-effects model with mean egg size as the response, including both components as fixed effects and female identity as a random intercept. The within-female coefficient estimates the plastic effect of egg number on egg size, whereas the between-female coefficient estimates the among-female (strategic) association.

To facilitate comparison of effect sizes, continuous predictors were standardized by centering them on their mean and scaling them by their standard deviation. In other words, each value was expressed relative to how far it deviated from the average, measured in units of standard deviations. The within-female and between-female components were each standardized using the mean and standard deviation of their respective distributions. When estimating standardized regression coefficients, the response variable was also scaled to have a standard deviation of one. As a result, standardized coefficients represent the expected change in egg size (measured in standard deviation units) associated with a one standard deviation change in the predictor.

To test whether females differed in their propensity to trade off egg size against egg number, I fitted a mixed-effects model that allowed the within-female effect of egg number to vary among females. The model included mean egg size as the response variable and partitioned egg number into two components: a within-female component (capturing how deviations from a female’s own average egg number affected egg size) and a between-female component (capturing differences among females in their average egg number). Female identity was included as a random effect with both random intercepts and random slopes for the within-female component of egg number. This approach estimates variation among females in their within-female slopes, as well as the correlation between intercepts and slopes, thereby quantifying heterogeneity in allocation strategies. Specifically, it allows assessment of whether females with larger average egg size differ systematically in the strength or direction of their within-female trade-off between egg number and egg size. The model was used to quantify individual variation in slopes but not to derive standardized fixed effects.
...
To compare clutch-specific variance components independently of random-slope parameterisation, we fitted a hierarchical Gaussian model in which female effects were estimated separately for first and second clutches. Scaled egg size was modelled as a function of clutch (first vs. second clutch) with clutch-specific random intercepts for female identity. This parameterisation estimates separate between-female variance components for clutch 1 and clutch 2, as well as their correlation, allowing direct comparison of female-level variation across reproductive events. Within-clutch variance was allowed to differ between broods by modelling the residual standard deviation as a function of clutch. Clutch-specific variance components were derived from posterior samples, and differences between clutches were evaluated using posterior contrasts.
...
#### Egg size variation between and within clutches
