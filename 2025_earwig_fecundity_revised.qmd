...
# Results
I did not support the prediction that egg number and egg size positively correlate either phenotypically (*r*~p~ [95% CrI] =  `r myround(pheno.corr.r[[1,1]],2)` [`r myround(pheno.corr.r[[1,2]],2)`, `r myround(pheno.corr.r[[1,3]],2)`]; @fig-one) or among females (*r*~among~ [95% CrI] = `r myround(tidy(trade_off.brms_cov.mass)[[9,6]],2)` [`r myround(tidy(trade_off.brms_cov.mass)[[9,8]],2)`, `r myround(tidy(trade_off.brms_cov.mass)[[9,9]],2)`]). Contrary to prediction, egg number and size were positively, rather than negatively, correlated at the within-individual level (*r*~within~ [95% CrI] = `r myround(res_cor[[1,1]],2)` [`r myround(res_cor[[1,2]],2)`, `r myround(res_cor[[1,3]],2)`]). Female body size is psotively correlated with first clutch size () but not the size of the second clutch ().
