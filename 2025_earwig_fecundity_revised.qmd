...
Both egg number and egg size declined in the second clutch In the multivariate Gaussian–Poisson model, egg size was smaller in clutch two relative to clutch one ($\beta$ = `r myround(m_mv_2.sum[3,5],2)`, 95% CrI (`r myround(m_mv_2.sum[3,7],2)`, `r myround(m_mv_2.sum[3,8],2)`). Egg number also declined significantly (log-scale β = $\beta$ = `r myround(m_mv_2.sum[4,5],2)`, 95% CrI (`r myround(m_mv_2.sum[4,7],2)`, `r myround(m_mv_2.sum[4,8],2)`), corresponding to an approximate 34% reduction on the natural scale ($e^{`r myround(m_mv_2.sum[4,5],3`)} \approx `r round(exp(-0.412), 2)`$). Among-female variation remained detectable for both egg size (SD = 0.091, 95% CrI [0.009, 0.165]) and egg number (SD = 0.355, 95% CrI [0.273, 0.460]), indicating persistent heterogeneity across broods.
