...
# revised manuscript. It treats the two observations as paired reproductive
# events and asks two principal questions:
#   1. Is egg size-number covariance expressed among or within females?
#   2. Does the association differ between initial and replacement events?
# Change-score and within-between models examine the sensitivity of the
# within-female interpretation to absolute versus proportional scales.
#
# Primary input: data/processed/df_complete_ingested.rds, generated by
# scripts/01_ingest_egg_data.R (one row per female x reproductive event)
...
if (
  ...
  anyNA(df[, c("id", "brood", "egg.number", "mean.egg.size", "mean.pro")]) ||
  any(df$egg.number <= 0) ||
